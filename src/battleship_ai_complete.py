...
class BattleshipHunter:
    ...
    def on_hit(self, ship, position):
        ...
        if ship in self.hits:  # TODO: self.hits.has(ship)
    ...
    def in_bounds(self, position):
        ...
        return x >= 0 and y >= 0 and x < len(self.board) and y < len(self.board[0])
...
class GameUI:
    ...
    def draw_stats(self):
        ...
        text = self.font.render(
            ...
            f"Accuracy: {round((self.game_manager.battleship_hunter.shots_hit / 
                                max(1, self.game_manager.battleship_hunter.total_shots)) * 100, 2)}%",
            True,
            (255, 255, 255)
        )
